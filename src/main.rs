...
use persistence::{decrypt_password, encrypt_password, load_state, save_state, AppState};
use terminal_ux::{format_message_display, print_help, ChatContext, ChatMode};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        ...
        println!(
            "\x1b[37m                bitch@ the terminal {}\x1b[0m",
            VERSION
        );
        println!(
            "\x1b[38;5;40m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\x1b[0m\n"
    ...
    let mut nickname = app_state
        .nickname
        .clone()
        .unwrap_or_else(|| "my-rust-client".to_string());
    ...
    peripheral
        .write(cmd_char, &announce_packet, WriteType::WithoutResponse)
        .await?;
        ...
        for (channel, encrypted_password) in &app_state.encrypted_channel_passwords {
            match decrypt_password(encrypted_password, identity_key) {
                Ok(password) => {
                    ...
                    debug_println!(
                        "[CHANNEL] Restored key for password-protected channel: {}",
                        channel
                    );
                }
                Err(e) => {
                        ...
                        match parse_bitchat_packet(&notification.value) {
                            Ok(packet) => {
                             ...
                             match packet.msg_type {
                                 ...
                                 MessageType::Announce => {
                                     let peer_nickname = String::from_utf8_lossy(&packet.payload).trim().to_owned();
                                     ...
                                     peer_entry.nickname = Some(peer_nickname.clone().to_string());


        //+        let pk_lossy = String::from_utf8_lossy(public_key_data);
        //+        let pid_lossy = String::from_utf8_lossy(public_key_data);
        //+
...
#[allow(dead_code)]
fn create_bitchat_message_payload_with_flags(
    sender: &str,
    content: &str,
    channel: Option<&str>,
    is_private: bool,
...
fn create_bitchat_message_payload_full(
    sender: &str,
    content: &str,
    channel: Option<&str>,
    is_private: bool,
...
fn parse_bitchat_packet(data: &[u8]) -> Result<BitchatPacket, &'static str> {
    ...
    let sender_id_str = String::from_utf8_lossy(&sender_id)
        .trim_end_matches('\0')
        .to_string();
    ...
    let (recipient_id, recipient_id_str) = if has_recipient {
        ...
        let recipient_id_str = String::from_utf8_lossy(&recipient_id)
            .trim_end_matches('\0')
            .to_string();
...
fn create_bitchat_packet_with_signature(
    sender_id_str: &str,
    msg_type: MessageType,
    payload: Vec<u8>,
...
fn create_bitchat_packet_with_recipient_and_signature(
    sender_id_str: &str,
...
fn create_bitchat_packet_with_recipient(
    sender_id_str: &str,
    ...
    if has_recipient {
        if let Some(recipient) = recipient_id_str {
            ...
            debug_full_println!(
                "[PACKET] Recipient ID (broadcast): {} bytes: {}",
                BROADCAST_RECIPIENT.len(),
                hex::encode(&BROADCAST_RECIPIENT)
            );
        }
    } else {
...
async fn send_packet_with_fragmentation(
    ...
    if packet.len() > 500 {
        ...
        println!(
            "[FRAG] Original packet hex (first 64 bytes): {}",
        ...
        for (index, chunk) in chunks.iter().enumerate() {
            ...
            println!(
                "[FRAG] Fragment header: ID={} index={:02X}{:02X} total={:02X}{:02X} type={:02X}",
            ...
            println!(
                "[FRAG] Fragment payload size: {} bytes",
            ...
            println!(
                "[FRAG] Fragment payload hex: {}",
                hex::encode(&fragment_payload)
            );
            println!("[FRAG] Fragment payload breakdown:");
            println!(
                "[FRAG]   Fragment ID (8 bytes): {}",
                hex::encode(&fragment_payload[0..8])
            );
            println!(
                "[FRAG]   Index (2 bytes): {} = {:02X}{:02X}",
                index, fragment_payload[8], fragment_payload[9]
            );
            println!(
                "[FRAG]   Total (2 bytes): {} = {:02X}{:02X}",
                total_fragments, fragment_payload[10], fragment_payload[11]
            );
            println!(
                "[FRAG]   Original type (1 byte): {} = {:02X}",
                MessageType::Message as u8,
                fragment_payload[12]
            );
            println!(
            ...
            println!(
                "[FRAG] Final fragment packet size: {} bytes",
                fragment_packet.len()
            );
            println!(
                "[FRAG] Final packet hex (first 64 bytes): {}",
            ...
            if let Ok(parsed_packet) = parse_bitchat_packet(&fragment_packet) {
                ...
                println!(
                    "[FRAG] Packet payload size: {} bytes",
                    parsed_packet.payload.len()
                );
                println!(
                    "[FRAG] Packet payload hex: {}",
            ...
            println!(
                "[FRAG] ✓ Fragment {}/{} sent successfully",
                index + 1,
                total_fragments
            );
