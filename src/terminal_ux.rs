...
impl ChatContext {
    ...
    pub fn switch_to_number(&mut self, num: usize) -> bool {
        ...
        if dm_idx < dm_vec.len() {
            ...
            println!("\x1b[90m─────────────────────────\x1b[0m");
            println!(
                "\x1b[90m» Switched to DM with {}. Just type to send messages.\x1b[0m",
                nick
            );
            return true;
        }
    ...
    pub fn enter_dm_mode(&mut self, nickname: &str, peer_id: &str) {
        ...
        println!("\x1b[90m─────────────────────────\x1b[0m");
        println!(
            "\x1b[90m» Entered DM mode with {}. Just type to send messages.\x1b[0m",
            nickname
        );
    }
    ...
    pub fn get_mode_info(&self) -> String {
        match &self.current_mode {
            ...
            ChatMode::Channel(ch) => {
                format!("Channel {} - messages visible to channel members", ch)
            }
            ChatMode::PrivateDM { nickname, .. } => {
                format!("Private chat with {} - end-to-end encrypted", nickname)
            }
        }
    }
