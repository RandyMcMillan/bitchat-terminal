...
impl EncryptionService {
    ...
    pub fn add_peer_public_key(
        ...
        if public_key_data.len() != 96 {
            println!(
                "[CRYPTO] Invalid public key data size: {}, expected 96",
                pk_lossy.len()
            );
            println!("[MESSAGE] {} {:?}, expected 96", pid_lossy, pk_lossy);
        ...
        let identity_key = match VerifyingKey::from_bytes(&identity_key_bytes) {
            ...
            Err(_) => {
                ...
                eprintln!(
                    "[CRYPTO] Note: Peer {} appears to be Android (invalid identity key format)",
                    peer_id
                );
                signing_key.clone()
            }
